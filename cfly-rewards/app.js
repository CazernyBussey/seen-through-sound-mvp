(() => {
  const cfg = {
    ...
    max: 7,
    delay: 700,
    files: { link: './assets/rewards/we-in-here-reward-placeholder.txt', name: 'we-in-here-cazerny-bussey-reward.txt' },
    items: [
      ['we-in-here', 'We In Here', '🎧', true],
      ['ny-basketball', 'New York basketball', '🏀', true],
      ['cfly-crown', 'CFLY crown', '👑', true],
      ['microphone', 'Microphone', '🎤', false],
      ['skyline', 'City skyline', '🌆', false],
  ...
  function start() {
    ...
    load();
    nodes.downloadTitle.textContent = 'You matched three! We in here!';
    nodes.downloadMessage.textContent = 'CFLY! Your fan download is unlocked. Replace the placeholder file with the official We In Here MP3 before public launch.';
    nodes.downloadLink.textContent = 'Download We In Here';
    nodes.downloadLink.href = cfg.files.link;
    nodes.downloadLink.setAttribute('download', cfg.files.name);
    nodes.shareLink.href = `https://twitter.com/intent/tweet?text=${encodeURIComponent('I just played CFLY Fan Unlock: We In Here Edition by Cazerny Bussey.')}&url=${encodeURIComponent(location.href)}`;
    ...
    draw();
    say('Welcome to CFLY Fan Unlock: We In Here Edition. Press play to begin.', true);
  }

  async function play() {
    if (state.busy) return;
    if (state.done) { openPanel(); say('Your fan download is already unlocked. CFLY forever loves you.', true); return; }
    ...
    state.used += 1;
    save(); draw();
    if (left() === 0) say('This is your last play for today.', true);
    moving(true);
    ...
    if (ok) { state.done = true; result(`Tile one is ${picked[0][1]}, tile two is ${picked[1][1]}, tile three is ${picked[2][1]}. CFLY! You matched three. We in here. Your fan download is unlocked.`); tone('high'); openPanel(); say('CFLY! You matched three. We in here. Your fan download is unlocked.', true, true); }
    else { result(`${picked.map((x,i)=>`tile ${i+1} is ${x[1]}`).join(', ')}. Not this play. Run it back. You have ${left()} ${left() === 1 ? 'play' : 'plays'} left today.`); tone('low'); }
    state.busy = false;
    save(); draw();
